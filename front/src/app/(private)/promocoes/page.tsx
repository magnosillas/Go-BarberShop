...
export default function PromocoesPage() {
  ...
  const [selectedSaleForEmail, setSelectedSaleForEmail] = useState<Sale | null>(
    null,
  );
  ...
  async function searchByCoupon() {
    ...
    try {
      const res = await generica({
        metodo: "GET",
        uri: `/sale/coupon/${couponSearch.trim()}`,
      });
      if (res?.data) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.name) {
      toast.error("Preencha o nome da promoção");
      return;
    }
  ...
  return (
    <GoBarberLayout>
      <div className="space-y-6">
        ...
        <div className="gobarber-card">
          ...
          <div className="flex gap-2">
            ...
            />
            <button
              onClick={searchByCoupon}
              disabled={searchingCoupon}
              className="gobarber-btn-primary px-4"
            >
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-semibold text-[#1A1A2E]">
                    {couponResult.name || couponResult.title}
                  </p>
                  <p className="text-sm text-gray-500">
                    {couponResult.description}
                  </p>
                  {couponResult.coupon && (
                    <code className="bg-white px-2 py-0.5 rounded text-xs font-mono text-[#E94560]">
                      {couponResult.coupon}
                    </code>
                  )}
                </div>
                {couponResult.totalPrice != null &&
                  couponResult.totalPrice > 0 && (
                    <span className="text-lg font-bold text-[#E94560]">
                      R$ {couponResult.totalPrice.toFixed(2)}
                    </span>
                  )}
              </div>
            </div>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  {sale.totalPrice != null && sale.totalPrice > 0 && (
                    <span className="text-xl font-bold text-[#E94560] shrink-0 ml-2">
                  ...
                  {sale.discountPercentage != null &&
                    ...
                    (!sale.totalPrice || sale.totalPrice === 0) && (
                      <span className="text-2xl font-bold text-[#E94560] shrink-0 ml-2">
                ...
                <div className="text-xs text-gray-400 mb-3">
                  {sale.startDate && (
                    <>
                      De {new Date(sale.startDate).toLocaleDateString("pt-BR")}
                    </>
                  )}
                  {sale.endDate && (
                    <>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                  <span
                    className={`px-2 py-1 text-xs rounded-full ${isSaleActive(sale) ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                  >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              inputMode="numeric"
              value={formatCurrencyDisplay(form.totalPrice)}
              onChange={(e) =>
                setForm({
                  ...form,
                  totalPrice: parseCurrencyInput(e.target.value),
                })
              }
              className="gobarber-input"
              required
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Cupom{" "}
              <span className="text-xs text-gray-400">(7 caracteres)</span>
            </label>
            <input
              type="text"
              value={form.coupon}
      ...
      <Modal
        ...
        title="Enviar Promoção por Email"
      >
        <div className="space-y-5">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h4 className="font-semibold text-[#1A1A2E]">
                {selectedSaleForEmail?.name ||
                  selectedSaleForEmail?.title ||
                  "Promoção"}
              </h4>
            </div>
            ...
            {selectedSaleForEmail?.coupon && (
              <p className="text-sm">
                Cupom:{" "}
                <code className="bg-white px-2 py-0.5 rounded font-mono text-[#E94560]">
                  {selectedSaleForEmail.coupon}
                </code>
              </p>
            )}
            {selectedSaleForEmail?.totalPrice != null &&
              selectedSaleForEmail.totalPrice > 0 && (
          ...
          <div className="text-sm text-gray-600">
            <p>
              Esta promoção será enviada por email para{" "}
              <span className="font-semibold">
                todos os clientes cadastrados
              </span>{" "}
              que aceitam receber promoções.
            </p>
          </div>
      ...
      <Modal
        ...
        title={detailSale?.name || detailSale?.title || "Promoção"}
      >
        {detailSale && (
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-lg text-[#1A1A2E]">
                  {detailSale.name || detailSale.title}
                </h3>
                <span
                  className={`px-2 py-0.5 text-xs rounded-full ${isSaleActive(detailSale) ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                >
                  {isSaleActive(detailSale) ? "Ativa" : "Inativa"}
                </span>
              </div>
            </div>
            {detailSale.description && (
            ...
            <div className="grid grid-cols-2 gap-3">
              {detailSale.totalPrice != null && detailSale.totalPrice > 0 && (
                <div className="bg-gray-50 p-3 rounded-lg">
                  ...
                  <p className="text-lg font-bold text-[#E94560]">
                    R$ {detailSale.totalPrice.toFixed(2)}
                  </p>
                </div>
              )}
              {detailSale.discountPercentage != null &&
                detailSale.discountPercentage > 0 && (
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <p className="text-xs text-gray-500">Desconto</p>
                    <p className="text-lg font-bold text-green-600">
                      {detailSale.discountPercentage}%
                    </p>
                  </div>
                )}
              {detailSale.coupon && (
                <div className="bg-gray-50 p-3 rounded-lg">
                  ...
                  <code className="text-sm font-mono text-[#E94560]">
                    {detailSale.coupon}
                  </code>
                </div>
              )}
              <div className="bg-gray-50 p-3 rounded-lg">
                ...
                <p className="text-sm">
                  {detailSale.startDate
                    ? new Date(detailSale.startDate).toLocaleDateString("pt-BR")
                    : "—"}{" "}
                  →{" "}
                  {detailSale.endDate
                    ? new Date(detailSale.endDate).toLocaleDateString("pt-BR")
                    : "—"}
                </p>
              </div>
            </div>

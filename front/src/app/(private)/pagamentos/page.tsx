...
export default function PagamentosPage() {
  ...
  // Extended state
  const [activeTab, setActiveTab] = useState<"all" | "pending" | "stats">(
    "all",
  );
  ...
  async function loadPayments() {
    ...
    try {
      ...
      if (!response || response.status >= 400) {
        toast.error(
          response?.data?.message || "Nao foi possivel carregar pagamentos",
        );
        setPayments([]);
        return;
      }
  ...
  async function loadAppointmentOptions() {
    try {
      ...
      const valid = list.filter(
        (a: AppointmentOption) =>
          !["REJECTED", "CANCELLED"].includes((a.status || "").toUpperCase()),
      );
  ...
  function handleAppointmentChange(appointmentIdValue: string) {
    const selected = appointments.find(
      (a) => a.id === Number(appointmentIdValue),
    );
    setForm((prev) => ({
      ...prev,
      ...
      amount:
        prev.amount ||
        (selected?.totalPrice != null ? String(selected.totalPrice) : ""),
      clientId:
        prev.clientId || (selected?.clientId ? String(selected.clientId) : ""),
      barberId:
        prev.barberId ||
        (selected?.barber?.idBarber ? String(selected.barber.idBarber) : ""),
    }));
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.appointmentId || !form.amount || !form.paymentMethod) {
      toast.error("Selecione agendamento, valor e metodo de pagamento");
      return;
    }
    setSaving(true);
    try {
      ...
      if (form.commissionRate)
        payload.commissionRate = parseFloat(form.commissionRate);
      if (form.couponCode) payload.couponCode = form.couponCode;
      if (form.loyaltyPointsUsed)
        payload.loyaltyPointsUsed = form.loyaltyPointsUsed;
  ...
  async function confirmPayment(id: number) {
    try {
      const res = await generica({
        metodo: "POST",
        uri: `/payment/${id}/confirm`,
        params: { transactionId: `TXN-${Date.now()}` },
      });
      if (res?.status === 200) {
  ...
  async function refundPayment(id: number) {
    const { value: reason } = await Swal.fire({
      ...
      inputValidator: (value) => {
        if (!value) return "O motivo é obrigatório!";
      },
    });
    if (!reason) return;
    try {
  ...
  async function partialRefund(id: number) {
    const { value: amountStr } = await Swal.fire({
      ...
      cancelButtonText: "Cancelar",
      inputValidator: (value) => {
        if (!value || parseFloat(value) <= 0) return "Informe um valor válido!";
      },
    });
    if (!amountStr) return;
    const { value: reason } = await Swal.fire({
    ...
    try {
      const res = await generica({
        metodo: "POST",
        uri: `/payment/${id}/partial-refund`,
        params: {
          amount: parseFloat(amountStr),
          reason: reason || "Reembolso parcial",
        },
      });
      if (res?.status === 200) {
        toast.success("Reembolso parcial realizado!");
        await refreshCurrentList();
      } else toast.error("Erro ao reembolsar parcialmente");
    } catch {
      toast.error("Erro no reembolso parcial");
    }
  }
  ...
  async function loadPixCode(id: number) {
    try {
      const [codeRes, qrRes] = await Promise.all([
        generica({ metodo: "GET", uri: `/payment/${id}/pix-code` }).catch(
          () => null,
        ),
        generica({ metodo: "GET", uri: `/payment/${id}/pix-qrcode` }).catch(
          () => null,
        ),
      ]);
      setPixModal({
        code: codeRes?.data?.pixCode || codeRes?.data,
        qrcode: qrRes?.data?.qrCode || qrRes?.data,
      });
    } catch {
  ...
  async function loadByAppointment(appointmentId: number) {
    try {
      const res = await generica({
        metodo: "GET",
        uri: `/payment/appointment/${appointmentId}`,
      });
      const data = res?.data?.content || res?.data || [];
  ...
  async function loadByPeriod() {
    ...
    try {
      ...
      const res = await generica({
        metodo: "GET",
        uri: "/payment/period",
        params: { startDate, endDate, page: 0, size: 100 },
      });
      if (!res || res.status >= 400) {
        toast.error(
          res?.data?.message || "Nao foi possivel filtrar por periodo",
        );
        setPayments([]);
        return;
      }
      ...
      toast.error("Erro ao filtrar por período");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function loadRevenueStats() {
    try {
      const [total, today, month, daily, byMethod, ticket, count] =
        await Promise.all([
          generica({ metodo: "GET", uri: "/payment/revenue/total" }).catch(
            () => null,
          ),
          generica({ metodo: "GET", uri: "/payment/revenue/today" }).catch(
            () => null,
          ),
          generica({ metodo: "GET", uri: "/payment/revenue/month" }).catch(
            () => null,
          ),
          generica({ metodo: "GET", uri: "/payment/revenue/daily" }).catch(
            () => null,
          ),
          generica({ metodo: "GET", uri: "/payment/revenue/by-method" }).catch(
            () => null,
          ),
          generica({ metodo: "GET", uri: "/payment/average-ticket" }).catch(
            () => null,
          ),
      ...
      setRevenueByMethod(
        Array.isArray(byMethod?.data)
          ? byMethod.data
          : Object.entries(byMethod?.data || {}).map(([k, v]) => ({
              method: k,
              total: v,
            })),
      );
      setAvgTicket(
        typeof ticket?.data === "number"
          ? ticket.data
          : (ticket?.data?.averageTicket ?? 0),
      );
      setTotalCount(
  ...
  async function loadBarberRevenue(barberId: number) {
    try {
      const [revRes, commRes] = await Promise.all([
        generica({
          metodo: "GET",
          uri: `/payment/barber/${barberId}/revenue`,
        }).catch(() => null),
        generica({
          metodo: "GET",
          uri: `/payment/barber/${barberId}/commission`,
        }).catch(() => null),
      ]);
      setBarberCommissions([
        { barberId, revenue: revRes?.data, commission: commRes?.data },
      ]);
    } catch {
      /* silencioso */
    }
  ...
  const getOrigin = (p: Payment) =>
    p.appointmentId ? `Agendamento #${p.appointmentId}` : "Loja";
  const getPixImageSrc = (value?: string) => {
    ...
    const isLikelyBase64 =
      normalized.length > 120 && /^[A-Za-z0-9+/=\r\n]+$/.test(normalized);
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium ${activeTab === "all" ? "bg-[#1A1A2E] text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
          >
            Todos
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 ${activeTab === "pending" ? "bg-yellow-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
          >
            Pendentes{" "}
            {pendingCount > 0 && (
              <span className="px-2 py-0.5 rounded-full text-xs bg-white text-yellow-600 font-bold">
                {pendingCount}
              </span>
            )}
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 ${activeTab === "stats" ? "bg-green-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
          >
            <FaChartBar /> Receitas
          </button>
        </div>
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (e.target.value) loadByStatus(e.target.value);
                else loadPayments();
              }}
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (e.target.value) loadByMethod(e.target.value);
                else loadPayments();
              }}
              className="gobarber-input w-auto text-sm"
            >
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (e.target.value) loadByBarber(parseInt(e.target.value));
                else loadPayments();
              }}
              className="gobarber-input w-auto text-sm"
            >
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (e.target.value) loadByClient(parseInt(e.target.value));
                else loadPayments();
              }}
              className="gobarber-input w-auto text-sm"
            >
            ...
            <div className="flex items-center gap-2 ml-auto">
              ...
              <button
                ...
                className="px-3 py-1.5 bg-[#E94560] text-white rounded-lg text-sm"
              >
                Filtrar
              </button>
            </div>
        ...
        {activeTab === "stats" ? (
          <div className="space-y-6">
            ...
            {revenueByMethod.length > 0 && (
              <div className="gobarber-card">
                <h3 className="text-base font-semibold text-[#1A1A2E] mb-3 flex items-center gap-2">
                  <FaCreditCard /> Receita por Método
                </h3>
            ...
            {dailyRevenue.length > 0 && (
              <div className="gobarber-card">
                <h3 className="text-base font-semibold text-[#1A1A2E] mb-3 flex items-center gap-2">
                  <FaCalendarAlt /> Receita Diária (Últimos 14 dias)
                </h3>
            ...
            <div className="gobarber-card">
              <h3 className="text-base font-semibold text-[#1A1A2E] mb-3 flex items-center gap-2">
                <FaUserTie /> Receita/Comissão por Barbeiro
              </h3>
              <select
                onChange={(e) => {
                  if (e.target.value)
                    loadBarberRevenue(parseInt(e.target.value));
                }}
                className="gobarber-input w-auto mb-3"
              >
                  ...
                  <div key={i} className="grid grid-cols-2 gap-4">
                    ...
                    <div className="p-3 bg-blue-50 rounded-lg text-center">
                      ...
                      <p className="text-lg font-bold text-blue-600">
                        R${" "}
                        {Number(
                          typeof bc.commission === "number"
                            ? bc.commission
                            : bc.commission?.total || 0,
                        ).toFixed(2)}
                      </p>
                    </div>
                  </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-[#1A1A2E] font-semibold">
                          R$ {(p.finalAmount ?? p.amount)?.toFixed(2) || "0.00"}
                        </td>
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-1">
                            <button
                              onClick={() => viewPaymentDetail(pid)}
                              className="p-1.5 text-blue-600 hover:bg-blue-50 rounded"
                              title="Detalhes"
                            >
                              <FaEye />
                            </button>
                            {(p.paymentMethod || p.method)?.toUpperCase() ===
                              "PIX" && (
                              <button
                                onClick={() => loadPixCode(pid)}
      ...
      >
        {pixModal && (
          <div className="space-y-4 text-center">
            {getPixImageSrc(
              typeof pixModal.qrcode === "string" ? pixModal.qrcode : "",
            ) && (
            ...
            {pixModal.code && (
              <div className="bg-gray-50 rounded-lg p-3">
                ...
                <p className="text-sm font-mono break-all">
                  {typeof pixModal.code === "string"
                    ? pixModal.code
                    : JSON.stringify(pixModal.code)}
                </p>
                <button
                  onClick={() => {
                  ...
                  className="mt-2 px-3 py-1 bg-green-100 text-green-700 rounded text-sm flex items-center gap-1 mx-auto"
                >
                  <FaCopy /> Copiar
                </button>
              </div>
            ...
            <div>
              ...
              <input
                ...
                value={formatCurrencyDisplay(
                  parseFloat(String(form.amount)) || 0,
                )}
                onChange={(e) =>
                  setForm({
                    ...form,
                    amount: String(parseCurrencyInput(e.target.value)),
                  })
                }
                className="gobarber-input"
                required
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, cardBrand: e.target.value })
                  }
                  className="gobarber-input"
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <p className="font-medium">
                  {detailModal.appointmentId
                    ? `Agendamento #${detailModal.appointmentId}`
                    : "Loja"}
                </p>
              </div>
              <div>
                <span className="text-xs text-gray-500">Barbeiro</span>
                <p className="font-medium">{detailModal.barberName || "—"}</p>
              </div>
              <div>
                <span className="text-xs text-gray-500">Valor Bruto</span>
                <p className="font-medium">
                  R$ {detailModal.amount?.toFixed(2) || "0.00"}
                </p>
              </div>
              <div>
                <span className="text-xs text-gray-500">Valor Final</span>
                <p className="font-medium text-[#E94560]">
                  R${" "}
                  {(detailModal.finalAmount ?? detailModal.amount)?.toFixed(
                    2,
                  ) || "0.00"}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="font-medium">{detailModal.status || "—"}</p>
              </div>
              ...
              {detailModal.couponCode && (
                <div>
                  <span className="text-xs text-gray-500">Cupom</span>
                  <p className="font-medium">
                    {detailModal.couponCode} (-R${" "}
                    {detailModal.couponDiscount?.toFixed(2) || "0"})
                  </p>
                </div>
              )}
              {detailModal.commissionRate != null && (
                <div>
                  <span className="text-xs text-gray-500">Comissão</span>
                  <p className="font-medium">
                    {detailModal.commissionRate}% (R${" "}
                    {detailModal.commissionAmount?.toFixed(2) || "0"})
                  </p>
                </div>
              )}
              {detailModal.installments != null &&
                detailModal.installments > 1 && (
                  <div>
                    <span className="text-xs text-gray-500">Parcelas</span>
                    <p className="font-medium">{detailModal.installments}x</p>
                  </div>
                )}
              {detailModal.cardLastDigits && (
                <div>
                  <span className="text-xs text-gray-500">Cartão</span>
                  <p className="font-medium">
                    **** {detailModal.cardLastDigits}{" "}
                    {detailModal.cardBrand || ""}
                  </p>
                </div>
              )}
              <div>
                ...
                <p className="font-medium">
                  {detailModal.paymentDate || detailModal.createdAt
                    ? new Date(
                        detailModal.paymentDate || detailModal.createdAt!,
            ...
            {detailModal.notes && (
              <div>
                ...
                <p className="text-sm mt-1">{detailModal.notes}</p>
              </div>
            )}
          </div>
        )}

...
export default function SecretariasPage() {
  ...
  const [detailSecretary, setDetailSecretary] = useState<Secretary | null>(
    null,
  );
  const [profilePhoto, setProfilePhoto] = useState<string | null>(null);
  const [loggedSecretary, setLoggedSecretary] = useState<Secretary | null>(
    null,
  );
  ...
  useEffect(() => {
    loadSecretaries();
    if (hasRole("SECRETARY")) loadLoggedSecretary();
  }, []);
  ...
  async function loadSecretaries() {
    ...
    try {
      ...
      toast.error("Erro ao carregar secretárias");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function loadSecretaryPhoto(id: number) {
    try {
      const res = await generica({
        metodo: "GET",
        uri: `/secretary/${id}/profile-photo`,
        responseType: "blob",
      });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editingId) {
        ...
        const secretaryData = {
          ...sanitizedForm,
          salary: sanitizedForm.salary
            ? parseFloat(String(sanitizedForm.salary))
            : 0,
          workload: sanitizedForm.workload
            ? parseInt(String(sanitizedForm.workload), 10)
            : 0,
          contact: sanitizedForm.contact
            ? onlyDigits(sanitizedForm.contact)
            : "",
        };
        ...
        if (res?.status === 200) {
          toast.success("Secretária atualizada!");
          setModalOpen(false);
          loadSecretaries();
  ...
  async function handleDelete(id: number) {
    ...
    try {
      ...
      if (res?.status === 200 || res?.status === 204) {
        toast.success("Secretária excluída!");
        loadSecretaries();
      } else toast.error("Erro ao excluir");
    } catch {
      toast.error("Erro ao excluir secretária");
    }
  }
  ...
  return (
    <GoBarberLayout>
                ...
                secretaries.map((s) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-gray-600 hidden md:table-cell">
                      {s.contact ? formatPhoneBR(s.contact) : "—"}
                    </td>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => viewDetail(s.idSecretary)}
                        ...
                        <button
                          onClick={() => handleDelete(s.idSecretary)}
      ...
      <Modal
        ...
        title={editingId ? "Editar Secretária" : "Nova Secretária"}
      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                value={form.contact}
                onChange={(e) =>
                  setForm({ ...form, contact: formatPhoneBR(e.target.value) })
                }
                className="gobarber-input"
            ...
            <div>
              ...
              <input
                ...
                value={formatCurrencyDisplay(
                  parseFloat(String(form.salary)) || 0,
                )}
                onChange={(e) =>
                  setForm({
                    ...form,
                    salary: String(parseCurrencyInput(e.target.value)),
                  })
                }
                className="gobarber-input"
                required
      ...
      <Modal
        isOpen={!!detailSecretary}
        onClose={() => {
          setDetailSecretary(null);
          setProfilePhoto(null);
        }}
        title="Detalhe da Secretária"
      >
        {detailSecretary && (
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              {profilePhoto ? (
                ...
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#E94560] to-[#0F3460] flex items-center justify-center text-white text-xl">
                  <FaUser />
                </div>
              )}
              <div>
                <h3 className="font-semibold text-lg text-[#1A1A2E]">
                  {detailSecretary.name}
                </h3>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-gray-50 p-3 rounded-lg">
                ...
                <p className="font-medium">
                  {detailSecretary.contact
                    ? formatPhoneBR(detailSecretary.contact)
                    : "—"}
                </p>
              </div>
              <div className="bg-gray-50 p-3 rounded-lg">
                <p className="text-xs text-gray-500">CPF</p>
                <p className="font-medium">{detailSecretary.cpf || "—"}</p>
              </div>
              {detailSecretary.salary != null && (
                <div className="bg-gray-50 p-3 rounded-lg">
                  <p className="text-xs text-gray-500">Salário</p>
                  <p className="font-medium">
                    R$ {Number(detailSecretary.salary).toFixed(2)}
                  </p>
                </div>
              )}
              {detailSecretary.workload != null && (
                <div className="bg-gray-50 p-3 rounded-lg">
                  ...
                  <p className="font-medium">{detailSecretary.workload}h</p>
                </div>
              )}
              ...
              {detailSecretary.admissionDate && (
                <div className="bg-gray-50 p-3 rounded-lg">
                  <p className="text-xs text-gray-500">Data Admissão</p>
                  <p className="font-medium">{detailSecretary.admissionDate}</p>
                </div>
              )}
              {detailSecretary.address &&
                (detailSecretary.address.street ||
                  detailSecretary.address.city) && (
                  <div className="bg-gray-50 p-3 rounded-lg col-span-2">
                    <p className="text-xs text-gray-500">Endereço</p>
                    <p className="font-medium">
                      {[
                        detailSecretary.address.street,
                        detailSecretary.address.number
                          ? `nº ${detailSecretary.address.number}`
                          : null,
                        detailSecretary.address.neighborhood,
                        detailSecretary.address.city,
                        detailSecretary.address.state,
                        detailSecretary.address.cep,
                      ]
                        .filter(Boolean)
                        .join(", ")}
                    </p>

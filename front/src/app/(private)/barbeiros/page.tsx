...
export default function BarbeirosPage() {
  ...
  async function addServiceToBarber(barberId: number, serviceId: number) {
    try {
      const response = await generica({
        metodo: "POST",
        uri: "/barber/service",
        data: { idBarber: barberId, idServices: [serviceId] },
      });
  ...
  async function removeServiceFromBarber(barberId: number, serviceId: number) {
    ...
    try {
      const response = await generica({
        metodo: "POST",
        uri: "/barber/service/remove",
        params: { barber: barberId, service: serviceId },
      });
  ...
  function openEdit(barber: Barber) {
    setForm({
      ...
      admissionDate:
        barber.admissionDate || new Date().toISOString().split("T")[0],
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const cleanForm = {
      ...
      address: {
        ...form.address,
        cep: form.address.cep?.replace(/\D/g, "").substring(0, 8) || "",
      },
    };
    try {
      if (editingId) {
        const formData = new FormData();
        const barberBlob = new Blob([JSON.stringify(cleanForm)], {
        ...
        if (photoFile) {
          ...
          response = await generica({
            metodo: "POST",
            uri: "/barber/create-without-photo",
            data: cleanForm,
          });
        }
        if (response?.status === 200 || response?.status === 201) {
              ...
              >
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => viewBarberDetail(barber.idBarber)}
                    ...
                    <button
                      onClick={() => handleDelete(barber.idBarber)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                value={form.contato}
                onChange={(e) =>
                  setForm({ ...form, contato: formatPhoneBR(e.target.value) })
                }
                className="gobarber-input"
                maxLength={15}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({
                    ...form,
                    salary: parseCurrencyInput(e.target.value),
                  })
                }
                className="gobarber-input"
              />
            ...
            <div>
              ...
              <input
                ...
                value={form.workload}
                onChange={(e) =>
                  setForm({ ...form, workload: parseInt(e.target.value) || 0 })
                }
                className="gobarber-input"
              />
            </div>
                ...
                onChange={(e) => {
                  ...
                  const fmt =
                    digits.length > 5
                      ...
                      : digits;
                  setForm({ ...form, address: { ...form.address, cep: fmt } });
                }}
                className="gobarber-input"
              />
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
                {availableServices.map((s) => (
                  <label
                    key={s.id}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm cursor-pointer border transition-colors ${form.idServices.includes(s.id) ? "bg-[#E94560] text-white border-[#E94560]" : "bg-white text-gray-600 border-gray-200 hover:border-gray-400"}`}
                  >
                    <input
                      type="checkbox"
                      className="hidden"
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          setForm({
                            ...form,
                            idServices: [...form.idServices, s.id],
                          });
                        } else {
                          setForm({
                            ...form,
                            idServices: form.idServices.filter(
                              (id) => id !== s.id,
                            ),
                          });
                        }
        ...
        {detailModal && (
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              {barberPhoto ? (
                <img
                  src={barberPhoto}
                  alt="Foto"
                  className="w-20 h-20 rounded-full object-cover border-2 border-[#E94560]"
                />
              ) : (
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <p>
                <strong>CPF:</strong> {detailModal.cpf}
              </p>
              <p>
                <strong>Contato:</strong>{" "}
                {detailModal.contato ? formatPhoneBR(detailModal.contato) : "—"}
              </p>
              <p>
                <strong>Salário:</strong> R${" "}
                {detailModal.salary?.toFixed(2) || "0.00"}
              </p>
              <p>
                <strong>Expediente:</strong> {detailModal.start || "—"} às{" "}
                {detailModal.end || "—"}
              </p>
              <p>
                <strong>Admissão:</strong> {detailModal.admissionDate || "—"}
              </p>
              <p>
                <strong>Carga Horária:</strong> {detailModal.workload || "—"}
                h/sem
              </p>
            </div>
            {detailModal.address &&
              (detailModal.address.street || detailModal.address.city) && (
                <div className="bg-gray-50 p-3 rounded-lg">
                  <p className="text-xs text-gray-500 mb-1">Endereço</p>
                  <p className="font-medium text-sm">
                    {[
                      detailModal.address.street,
                      detailModal.address.number
                        ? `nº ${detailModal.address.number}`
                        : null,
                      detailModal.address.neighborhood,
                      detailModal.address.city,
                      detailModal.address.state,
                      detailModal.address.cep,
                    ]
                      .filter(Boolean)
                      .join(", ")}
                  </p>
            ...
            <div>
              ...
              {detailModal.services && detailModal.services.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {detailModal.services.map((s) => (
                    <span
                      key={s.id}
                      className="flex items-center gap-1 px-2 py-1 text-xs bg-blue-50 text-blue-600 rounded-full"
                    >
                      {s.name}
                      <button
                        onClick={() =>
                          removeServiceFromBarber(detailModal.idBarber, s.id)
                        }
              ...
              {availableServices.filter(
                (s) => !detailModal.services?.some((ds) => ds.id === s.id),
              ).length > 0 && (
                <div className="mt-3">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {availableServices
                      .filter(
                        (s) =>
                          !detailModal.services?.some((ds) => ds.id === s.id),
                      )
                      .map((s) => (
                        <button
                          key={s.id}
                          onClick={() =>
                            addServiceToBarber(detailModal.idBarber, s.id)
                          }
                          className="flex items-center gap-1 px-2 py-1 text-xs bg-green-50 text-green-600 rounded-full hover:bg-green-100"
                        >
